...
from nes2.api.responses import EntityListResponse, VersionListResponse, RelationshipListResponse
...
async def list_entities(
    query: Optional[str] = Query(None, description="Text query to search in entity names"),
    entity_type: Optional[str] = Query(None, description="Filter by entity type (person, organization, location)"),
    sub_type: Optional[str] = Query(None, description="Filter by entity subtype"),
    attributes: Optional[str] = Query(None, description="Filter by attributes (JSON object)"),
...
async def get_entity(
    entity_id: str = Path(..., description="Entity ID (e.g., entity:person/ram-chandra-poudel)"),
    search_service: SearchService = Depends(get_search_service)
):
...
async def get_entity_relationships(
    ...
    currently_active: Optional[bool] = Query(None, description="Filter for currently active relationships"),
    limit: int = Query(100, ge=1, le=1000, description="Maximum number of relationships"),
    ...
    try:
        ...
        logger.error(f"Error retrieving relationships for {entity_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error": {
